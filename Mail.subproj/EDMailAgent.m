...
- (void)sendMailWithHeaders:(NSDictionary *)userHeaders body:(NSString *)body andAttachments:(NSArray *)attachmentList
{
    ...
    if([recipients count] == 0)
        [NSException raise:NSInvalidArgumentException format:@"-[%@ %@]: No recipients in header list.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
    ...
    if((sender = [userHeaders objectForKey:EDMailSender]) == nil)
        ...
        if([authors count] == 0)
            [NSException raise:NSInvalidArgumentException format:@"-[%@ %@]: No sender or from field in header list.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
        if([authors count] > 1)
            [NSException raise:NSInvalidArgumentException format:@"-[%@ %@]: Multiple from addresses and no sender field in header list.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
...
- (void)sendMessage:(EDInternetMessage *)message
{
    ...
    if((newRecipients = [message bodyForHeaderField:EDMailBCC]) != nil)
        [recipients addObjectsFromArray:[newRecipients addressListFromEMailString]];
    if([recipients count] == 0)
        [NSException raise:NSInvalidArgumentException format:@"-[%@ %@]: No recipients in message headers.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
    ...
    if((sender = [message bodyForHeaderField:EDMailSender]) == nil)
        ...
        if([authors count] == 0)
            [NSException raise:NSInvalidArgumentException format:@"-[%@ %@]: No sender or from field in message headers.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
        if([authors count] > 1)
            [NSException raise:NSInvalidArgumentException format:@"-[%@ %@]: Multiple from addresses and no sender field in message headers.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
        sender = [authors objectAtIndex:0];
        }
    ...
    if([[message contentTransferEncoding] isEqualToString:MIME8BitContentTransferEncoding])
        ...
        if([stream handles8BitBodies] == NO)
            [NSException raise:NSInvalidArgumentException format:@"-[%@ %@]: Message has 8-bit content transfer encoding but remote MTA only accepts 7-bit.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
        }
    transferData = [message transferData];
