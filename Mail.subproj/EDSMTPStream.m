...
- (void)_assertServerIsReady
{
    ...
    if(state == ServerReadyForCommand)
	    ...
	    if(([self allowsPipelining] == NO) && ([self hasPendingResponses]))
			{
			[NSException raise:NSInternalInconsistencyException format:@"-[%@ %@]: Attempt to issue an SMTP command when a response is pending and server is not known to allow pipelining.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
			}
		}
	...
	if(state > ServerReadyForCommand)
        ...
        if(state == ServerReadingBody)
            {
            [NSException raise:NSInternalInconsistencyException format:@"-[%@ %@]: Attempt to issue an SMTP command within message body.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
            }
        else if(state == ConnectionClosed)
            {
            [NSException raise:NSInternalInconsistencyException format:@"-[%@ %@]: Attempt to issue an SMTP command on a closed stream.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
            }
        else
            {
            [NSException raise:NSInternalInconsistencyException format:@"-[%@ %@]: Attempt to issue an SMTP command on an unusable stream.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
            }
...
{
    if(state != ServerReadingBody)
        [NSException raise:NSInternalInconsistencyException format:@"-[%@ %@]: Attempt to finish a message body that was not begun.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
    [self writeString:@"\r\n.\r\n"];
    [pendingResponses addObject:@"250"];
    state = ServerReadyForCommand;
}
...
- (void)assertServerAcceptedCommand
{
    ...
    if([pendingResponses count] == 0)
        [NSException raise:NSInternalInconsistencyException format:@"-[%@ %@]: No pending responses.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
