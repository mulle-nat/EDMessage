...
- (NSData *)transferData
{
    ...
    if((headerData = [stringBuffer dataUsingEncoding:NSASCIIStringEncoding]) == nil)
        [NSException raise:NSInternalInconsistencyException format:@"-[%@ %@]: Transfer representation of header fields contains non ASCII characters.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
...
{
    return [NSString stringWithFormat:@"<%@ 0x%x: %@>", NSStringFromClass(EDObjcIsa), self, [self bodyForHeaderField:@"content-type"]];
}
...
- (void)_takeContentDataFromOriginalTransferData
{
    ...
    if(originalTransferData == nil)
        [NSException raise:NSInternalInconsistencyException format:@"-[%@ %@]: Original transfer data not available anymore.", NSStringFromClass(EDObjcIsa), NSStringFromSelector(_cmd)];
